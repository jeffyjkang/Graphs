...
# from created world invoke loadGraph and pass in the roomGraph, to create rooms
world.loadGraph(roomGraph)
# Create Player and pass in string "Name: to name and the startingRoom from
# created world which is initialized to room 0 as arguments, assign to player
player = Player("Name", world.startingRoom)


# FILL THIS IN
# traversal path or steps we are taking to visit the rooms, list will be updated when player actually moves
traversalPath = []
# traversalPath = ['n', 's']
# graph is a dict that will hold the possible directions of the room, update when the next room is visited
# update previous room's ?
graph = {}
# graph = {
#     0: {'n': 4, 's': 8: 'w': 3, 'e': '?'}
#     4: {'s': 0}
...
while len(graph) < 500:
    # assign currentRoomExits to already initialized graph,
    # will loop through each iteration and update graph
    currentRoomExits = graph[player.currentRoom.id]
    # print(f'current room exits: {currentRoomExits}')
    # initialize depth first traversal stack and pop while stack size is greater than 0
    dirToMove = []
    # find an existing direction in currentRoomExits/ graph at currentRoom id
    for directions in currentRoomExits:
        # print(f'directions: {directions}')
        # if the currentRoomsExits directions = '?' append it to the stack
        if currentRoomExits[directions] == '?':
            # append the dirToMove stack with the direction
            dirToMove.append(directions)
            # print(f'dirtomove: {dirToMove}')
    # while the stack is not empty traverse the maze
    if len(dirToMove) > 0:
        # assign direction from the end of the dirToMove stack
        direction = dirToMove.pop()
        # initialize previousRoom to current room id so we can fill the directions
        previousRoom = player.currentRoom.id
        # print(f'previousRoom: {previousRoom}')
        # invoke the player travel method by passing in the direction, to move to new room
        player.travel(direction)
        # print(player.currentRoom)
        # append the direction moved to the traversal path
        traversalPath.append(direction)
        # invoke new dict so that we can assign '?' to new room exits
        newDict = {}
        # only add the exits if current room is not in the graph
        if player.currentRoom.id not in graph:
            # for each available exit in current room, get exits
            for exits in player.currentRoom.getExits():
                 # assign the exits with questionmark in the newly created dict
                newDict[exits] = '?'
            # assign graph at current room id to newly created dict that has the exits
            graph[player.currentRoom.id] = newDict
        # assign the value of current room id to the graph at previous room id's direction
        graph[previousRoom][direction] = player.currentRoom.id
        # print(f'current room: {player.currentRoom.id}')
        # assign oppositeDirection to the value of the direction key in inverseDirections
        oppositeDirection = inverseDirections[direction]
        # assign the value of previous room id to the graph at current room id's direction
        graph[player.currentRoom.id][oppositeDirection] = previousRoom
        # print(f'previous room: {previousRoom})
    # if there are no more places to go, invoke bfs to find location of room with '?'
    else:
        # invoke the bfs function, pass in graph and current room id,
        # assign it to bfs path to get the shortest path to room with '?'
        bfs_path = find_nearest_unexplored(graph, player.currentRoom.id)
        # initialize room to first room or zeroith index of the bfs_path
        room = bfs_path[0]
        # loop through the paths starting at index of 1 to compare next rooms exit value
        for path in bfs_path[1:]:
            # find the value of the exits of the current room in the path
            for exits in graph[room]:
                # if the current room's exit value equals path, or the next room travel and append
                if graph[room][exits] == path:
                    # back_directions.append(exits)
                    # have player travel in the direction of exits
                    player.travel(exits)
                    # print(f'current loc: {player.currentRoom}')
                    # append the exits direction to the traversal path
